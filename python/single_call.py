...
def _gen_ai_attr(name: str, default: str) -> str:
    """Use semconv constant when present; else stable OTEL GenAI attribute name string."""
    return getattr(GenAI, name, default)


GEN_AI_OPERATION_NAME = _gen_ai_attr("GEN_AI_OPERATION_NAME", "gen_ai.operation.name")
GEN_AI_PROVIDER_NAME = _gen_ai_attr("GEN_AI_PROVIDER_NAME", "gen_ai.provider.name")
GEN_AI_REQUEST_MODEL = _gen_ai_attr("GEN_AI_REQUEST_MODEL", "gen_ai.request.model")
GEN_AI_RESPONSE_MODEL = _gen_ai_attr("GEN_AI_RESPONSE_MODEL", "gen_ai.response.model")
GEN_AI_USAGE_INPUT_TOKENS = _gen_ai_attr(
    "GEN_AI_USAGE_INPUT_TOKENS", "gen_ai.usage.input_tokens"
)
GEN_AI_USAGE_OUTPUT_TOKENS = _gen_ai_attr(
    "GEN_AI_USAGE_OUTPUT_TOKENS", "gen_ai.usage.output_tokens"
)
GEN_AI_RESPONSE_ID = _gen_ai_attr("GEN_AI_RESPONSE_ID", "gen_ai.response.id")
GEN_AI_RESPONSE_FINISH_REASONS = _gen_ai_attr(
    "GEN_AI_RESPONSE_FINISH_REASONS", "gen_ai.response.finish_reasons"
)
GEN_AI_INPUT_MESSAGES = _gen_ai_attr("GEN_AI_INPUT_MESSAGES", "gen_ai.input.messages")
GEN_AI_OUTPUT_MESSAGES = _gen_ai_attr("GEN_AI_OUTPUT_MESSAGES", "gen_ai.output.messages")
GEN_AI_TOOL_NAME = _gen_ai_attr("GEN_AI_TOOL_NAME", "gen_ai.tool.name")
GEN_AI_TOOL_DESCRIPTION = _gen_ai_attr("GEN_AI_TOOL_DESCRIPTION", "gen_ai.tool.description")
GEN_AI_TOOL_CALL_ID = _gen_ai_attr("GEN_AI_TOOL_CALL_ID", "gen_ai.tool.call.id")
GEN_AI_TOOL_TYPE = _gen_ai_attr("GEN_AI_TOOL_TYPE", "gen_ai.tool.type")

# Not in gen_ai_attributes in some semconv versions; use literals
...
def main() -> None:
    ...
    try:
        with tracer.start_as_current_span(
            ...
            chat_span.set_attribute(GEN_AI_OPERATION_NAME, operation_name)
            chat_span.set_attribute(GEN_AI_PROVIDER_NAME, "openai")
            chat_span.set_attribute(GEN_AI_REQUEST_MODEL, model)
            chat_span.set_attribute(GEN_AI_RESPONSE_MODEL, model)
            chat_span.set_attribute(GEN_AI_USAGE_INPUT_TOKENS, input_tokens)
            chat_span.set_attribute(GEN_AI_USAGE_OUTPUT_TOKENS, output_tokens)
            chat_span.set_attribute(GEN_AI_RESPONSE_ID, response_id)
            ...
            chat_span.set_attribute(
                GEN_AI_INPUT_MESSAGES, json.dumps(input_messages)
            )
            chat_span.set_attribute(
                GEN_AI_OUTPUT_MESSAGES, json.dumps(output_messages)
            )
            ...
            with tracer.start_as_current_span(
                ...
                tool_span.set_attribute(GEN_AI_TOOL_NAME, tool_name)
                tool_span.set_attribute(
                    GEN_AI_TOOL_DESCRIPTION,
                    "Get the current weather in a given location",
                )
                tool_span.set_attribute(GEN_AI_TOOL_CALL_ID, tool_call_id)
                tool_span.set_attribute(GEN_AI_TOOL_TYPE, "function")
                tool_span.set_attribute(
